	...
	uploadImage: function(param, callback) {
		$(document)
			...
			.on('click', param.id, function(event) {
				...
				common.open({
					title: param.title || label.uploadimages,
					shadeClose: false,
					content: _tmpl,
					success: function(layero, layerIndex) {
						...
						uploader.on('uploadStart', function(file) {
							$("#filePicker").text("").append('<i class="fa fa fa-spinner fa-pulse fa-fw"></i>"+label.uploading+"').attr('disabled', true);
						});
						// 当有文件添加进来的时候
						...
						uploader.on('uploadSuccess', function(file, response) {
							...
							common.msg(file.name + label.uploadsuccess);
						...
						uploader.on('error', function(type) {
							...
							switch (type) {
								case "Q_EXCEED_NUM_LIMIT":
									...
									break;
								case "F_EXCEED_SIZE":
									msg = label.uploadsize5;
									break;
								default:
									msg = label.uploadonlyphoto;
									break;
							}
							common.msg(msg);
						});
