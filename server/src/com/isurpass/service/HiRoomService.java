...
public class HiRoomService extends BaseService<Room>
{
	...
	public void saveRoom(Room room,PersonVo sessionPersonVo){
		if(PersonController.checkIfSecondAdmin(sessionPersonVo)){
			room.setPersonId(sessionPersonVo.getSuperpersonid().longValue());
		}else{
...
//		room.setDeviceCount(0);
		...
		room.setTypeinpersonid(sessionPersonVo.getId().intValue());
		super.save(room);
		...
		/**保存日志*/
		hiOperateLogService.saveOperateLog(7, "【" + dbDistrict.getDistrictName() + "】小区添加房间【" + room.getRoomName() + "】", sessionPersonVo.getRealName(), personid,operatepersonid, 
	 ...
	 * @param sessionPersonVo
	 */
	public void updateRoom(Room room,PersonVo sessionPersonVo, ResourceBundle rb){
		...
		HiDistrictService hiDistrictService = super.createService(HiDistrictService.class);
		District dbDistrict = hiDistrictService.findById(room.getDistrictId());
		hiOperateLogService.saveOperateLog(8, "【" + dbDistrict.getDistrictName() + "】小区修改房间【" + room.getRoomName() + "】", sessionPersonVo.getRealName(), personid,operatepersonid, 
				sessionPersonVo.getIp(), room.getId(),dbDistrict.getDistrictName(),room.getRoomName(),null);
	}
	...
	public List<Room> findRoom(RoomVo roomVo,List<Long> idList)
	{
		if(idList.size()==0)
			idList.add((long)-1);
		CriteriaWrap cw = super.createCriteriaWrap();
		cw.addifNotNull(ExpWrap.in("districtId", idList));
		cw.addifNotNull(ExpWrap.eq("districtId", roomVo.getDistrictId()));
		cw.addifNotNull(ExpWrap.eq("build", roomVo.getBuild()));
		cw.addifNotNull(ExpWrap.eq("unit", roomVo.getUnit()));
		cw.addifNotNull(ExpWrap.eq("floor", roomVo.getFloor()));
	 ...
	 * @return
	 */
	public List<RoomVo> findPage(RoomVo roomVo,BasicScroll scroll,List<Long> idList)
	{
		if(idList.size()==0)
			idList.add((long)-1);
		CriteriaWrap cw = super.createCriteriaWrap();
	...
	public List<Map> findBuildByDistrictId(Long personId,Long districtId)
	{
		...
		hw.add("where districtId = ? " , districtId ) ;
...
//		hw.add("where personId = ? " , personId ) ;
	 ...
	 * @param sessionPersonVo
	 */
	public void deleteRoom(Long id,PersonVo sessionPersonVo, ResourceBundle rb){
		Room dbRoom = this.findById(id);
		
		Room.checkSecondAdminRole(dbRoom, sessionPersonVo,rb);
		if(dbRoom.getGrantFlag() == 1){
			throw new BusinessException(rb.getString("authorizedroomcannotbedelete"));
		}
		HiDeviceService hiDeviceService = super.createService(HiDeviceService.class);
		int bindDeviceCount = hiDeviceService.findBindDeviceCount(id,null , null);
		if(bindDeviceCount > 0)
			throw new BusinessException(rb.getString("deletedeviceinroomfirst"));
		...
		hiOperateLogService.saveOperateLog(9, "【" + dbDistrict.getDistrictName() + "】小区删除房间【" + dbRoom.getRoomName() + "】", sessionPersonVo.getRealName(), personid,operatepersonid, 
	...
	public void grant(Long id,String grantUserName,String grantRealName,Date grantBeginDate,Date grantEndDate, PersonVo sessionPersonVo, ResourceBundle rb){
		...
		if(bindDeviceList.size() == 0)
			throw new BusinessException(rb.getString("binddevicefirst"));
		super.update(dbRoom);
	 ...
	 * @return
	 */
	public List<Map> findGrantRealName(Long personId,List<Long> idList)
	{
		if(idList.size()==0)
			idList.add((long)-1);
		...
		hw.add("where r.id = rp.roomId ");
		hw.addifnotnull("and r.districtId in :idList ","idList", idList);
		hw.setResultBeanClass(Map.class);
		return hw.list();
	}
	...
	public void recharge(Room room,Integer rechargetype,PersonVo sessionPersonVo)
	{
		...
		hiOperateLogService.saveOperateLog(22, "【" + dbDistrict.getDistrictName() + "】小区房间【" + dbRoom.getRoomName() + "】充值" + room.getBalance() +"分", sessionPersonVo.getRealName(), personid,operatepersonid, 
				sessionPersonVo.getIp(), dbRoom.getId(),dbDistrict.getDistrictName(),dbRoom.getRoomName(),room.getBalance()+"");
		
	}
	
	public List<RoomVo> queryRoomFee(RoomVo roomvo, Boolean arrearageonly,List<Long> idList)
	{
		List<Room> lst = this.findRoom(roomvo,idList);
	...
	public void withdraw(Room room,PersonVo sessionPersonVo, ResourceBundle rb)
	{
		Room dbRoom = this.findById(room.getId());
		
		if ( dbRoom.getBalance() < room.getBalance())
			throw new BusinessException(rb.getString("refundexception"));
		...
		HiDistrictService districtService = (HiDistrictService)this.applicationContext.getBean("HiDistrictService");
		District dbDistrict = districtService.findById(dbRoom.getDistrictId());
		hiOperateLogService.saveOperateLog(23, "【" + dbDistrict.getDistrictName() + "】小区房间【" + dbRoom.getRoomName() + "】退款" + room.getBalance() +"分", sessionPersonVo.getRealName(), personid,operatepersonid, 
				sessionPersonVo.getIp(), dbRoom.getId(),dbDistrict.getDistrictName(),dbRoom.getRoomName(),room.getBalance()+"");
	}
}
